...
- ✅ 纯 Rust 实现，无任何 Windows 依赖
- ✅ 通过 `pkg-config` 自动链接系统 libfluidsynth（无需编译期下载）
- ✅ 自动搜索系统 SoundFont（`.sf2`/`.sf3`），也可手动指定
- ✅ **直接播放 MIDI 文件**（`-m` 或 `.mid` 扩展名）：fluidsynth 原生多轨同步 + tempo 变速，最准确
- ✅ **改进版多音轨 TXT 格式**：支持任意数量并行音轨 + 轨内顺序段落
- ✅ 完全兼容原版 v1/v2 格式（左右手双轨、空行分组）
- ✅ `fluid_sequencer` 毫秒级精确事件调度
- ✅ **动态进度条**：播放时实时显示进度百分比、已播/总时长、剩余时间
- ✅ **峰值限制器（默认 -1dBFS）**：自动防止多音符叠加削波/电流声
- ✅ 调试模式：详细输出解析日志与每个 MIDI 事件
- ✅ 配套 Python 脚本：`MIDI → 简谱 TXT` 转换器
...
├── build.rs             # 构建脚本（pkg-config 自动链接 fluidsynth）
├── src/
│   ├── main.rs          # CLI 入口
│   ├── parser.rs        # 简谱 TXT 解析器（多音轨）
│   ├── synth.rs         # fluidsynth FFI 封装 + SoundFont 搜索
│   └── log.rs           # 日志模块
└── convert/
    └── convert.py       # MIDI → 简谱 TXT 转换器（纯 Python，无依赖）
```
...
./target/release/music -m 歌曲.mid          # 或显式指定 -m
...
./target/release/music 乐曲.mid -b 90       # MIDI 也支持速度覆盖
